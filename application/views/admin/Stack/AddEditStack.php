...
<script>
    // =============================================
    // Load Chambers when Godown changes
    // =============================================
    function loadChambersByGodown(GodownID, selectedChamberID) {
    var $chamber = $('#ChamberName');

    // Reset chamber dropdown
    ...
    $.ajax({
        url: "<?php echo admin_url(); ?>Stack/GetChambersByGodown",
        dataType: "JSON",
        method: "POST",
        ...
        success: function (data) {
            ...
            $.each(data, function (i, ch) {
                $chamber.append(
                    $('<option>', { 
                        value: ch.id,          // numeric id
                        text: ch.ChamberName 
                    })
                );
            });

            // Refresh first so selectpicker sees the new options
            $chamber.selectpicker('refresh');

            // Then set selected value — force string comparison
    ...
    // Trigger on Godown dropdown change
    $('#GodownName').on('change', function() {
        loadChambersByGodown($(this).val(), null);
    });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // =========================
                        ...
                        loadChambersByGodown(data.GodownName, data.ChamberName);
...
<style>
    ...
    .table-StackMaster_List thead th {
        position: sticky;
        top: 0;
        z-index: 1;
    }

    .table-StackMaster_List tbody th {
        position: sticky;
        left: 0;
    }
