          ...
          <div class="panel-body">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
                ...
                <li class="breadcrumb-item active" aria-current="page"><b>Delivery Order List</b></li>
              </ol>
            </nav>
            <hr class="hr_style">
                ...
                <div class="table-list" id="printableArea">
                  <table class="table table-striped table-bordered table-list" id="table-list" width="100%">
                    <thead>
                      ...
                      <tr>
                        <th class="sortable">Delivery Order No</th>
...
<script>
  ...
  $('#filter_list_form').submit(function(e) {
    ...
    function fetchChunk() {
      ...
      $.ajax({
        url: "<?= admin_url('DeliveryOrder/ListFilter') ?>",
        type: "POST",
        data: form_data,
        processData: false,
        contentType: false,
  ...
  function appendRows(rows) {
    ...
    rows.forEach(function(row) {
      html += `<tr>
        ...
        <td>${moment(row.DODate).format('DD/MM/YYYY')}</td>
        ...
        <td>${row.RoundOff || ''}</td>
        <td>${row.NetAmt || ''}</td>
        <td class="text-center">${row.Status || 'Pending'}</td>
      </tr>`;
    });
  ...
  function exportTableToExcel() {
    ...
    $.ajax({
      url: "<?= admin_url('DeliveryOrder/ListExportExcel') ?>",
      type: 'POST',
      data: formData,
      dataType: 'json',
      processData: false,
