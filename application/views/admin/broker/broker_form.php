						...
						<div class="row">
							<div class="col-md-2">
								<div class="form-group">
									<label class="control-label"><small class="req text-danger">* </small>Broker Group</label>
									<select class="selectpicker form-control" id="groups_in"
										...
										<option value=""></option>
										<?php foreach ($getbrokergroups as $key => $value) { ?>
											<option
							...
							<div class="col-md-2">
								<div class="form-group">
									<input type="hidden" id="HiddenBrokerCode" name="HiddenBrokerCode">
									<label class="control-label"><small class="req text-danger">* </small>Broker Code</label>
							...
							<div class="col-md-4">
								<div class="form-group">
									<label class="control-label"><small class="req text-danger">* </small>Broker Name</label>
						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<select name="vendor" id="vendor" class="selectpicker form-control" data-live-search="true" required>
										<option value=""></option>
							...
							<div class="col-md-3">
								<div class="form-group">
									<label class="control-label"><small class="req text-danger">* </small>Customer</label>
									<select name="customer" id="customer" class="selectpicker form-control" data-live-search="true" required>
										<option value=""></option>
							...
							<div class="col-md-4">
								<div class="form-group" id='BlockedReasonDiv'>
									<label class="control-label"><small class="req text-danger">* </small>Blocked Reason</label>
									<textarea name="blocked_reason" id="blocked_reason" class="form-control" rows="2"></textarea>
								</div>
							</div>
							...
							<div class="modal-dialog modal-lg" role="document">
								<div class="modal-content">
									<div class="modal-header" style="padding:5px 10px;">
										...
										<h4 class="modal-title">Broker List</h4>
									</div>
									<div class="modal-body" style="padding:0px 5px !important">
										...
										<div class="table-Account_List tableFixHead2">
											<table
												...
												<thead>
													<tr>
														<th style="text-align:left;" class="sortablePop">Broker Code</th>
														<th style="text-align:left;" class="sortablePop">Broker Name</th>
														...
														<th style="text-align:left;" class="sortablePop">GST Category</th>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#AccountID").dblclick(function() {
			$('#Account_List').modal('show');
			$.ajax({
				url: "<?php echo admin_url(); ?>Broker/GetAllBrokerList",
				dataType: "JSON",
				method: "POST",
				beforeSend: function() {
				...
				success: function(data) {
					...
					$('.get_AccountID').on('click', function() {
						...
						$.ajax({
							url: "<?php echo admin_url(); ?>Broker/GetComprehensiveAccountData",
							dataType: "JSON",
							method: "POST",
							data: {
								AccountID: AccountID
		...
		function PopulateFormFromComprehensiveData(comprehensiveData) {
			try {
				...
				if (comprehensiveData && comprehensiveData.clientDetails) {
					...
					$('#AccountName').val(client.company || '');
				...
				if (comprehensiveData && comprehensiveData.bankData && comprehensiveData.bankData.length > 0) {
					...
					// Populate Vendor & Customer from brokerData by index
					if (comprehensiveData && comprehensiveData.brokerData && comprehensiveData.brokerData.length > 0) {

						let brokerRows = comprehensiveData.brokerData;

						// First row → Vendor
						if (brokerRows[0]) {
							$('#vendor').val(brokerRows[0].AccountID || '');
							console.log('Vendor populated:', brokerRows[0].AccountID);
						}

						// Second row → Customer
						if (brokerRows[1]) {
							$('#customer').val(brokerRows[1].AccountID || '');
							console.log('Customer populated:', brokerRows[1].AccountID);
						}

						$('.selectpicker').selectpicker('refresh');
					}

				// //6. Populate attachment if exists
...
if (client.Attachment && client.Attachment.trim() !== '') {
	...
	$('#groups_in').on('change', function() {
		var selectedBrokerType = $(this).val();

		// Clear if nothing selected
		if (!selectedBrokerType) {
			$('#AccountID').val('');
			$('#HiddenBrokerCode').val('');
			return;
		}
		...
		$.ajax({
			url: "<?php echo admin_url('Broker/GetNextBrokerCode'); ?>",
			method: 'POST',
			dataType: 'json',
			data: {
				ActSubGroupID2: selectedBrokerType
			},
			success: function(response) {
				if (response && response.status === 'success' && response.next_code) {
					$('#AccountID').val(response.next_code);
					$('#HiddenBrokerCode').val(response.ActSubGroupID2 || selectedBrokerType);
					console.log('Broker Code generated: ' + response.next_code + ' (Broker: ' + (
							response.broker_name || '') + ', Count: ' + (response.count || '') +
						')');
				} else {
					$('#AccountID').val('');
					$('#HiddenBrokerCode').val('');
					alert_float('warning', (response && response.message) ? response.message :
						'Failed to generate broker code');
				}
			},
			error: function(xhr) {
				console.error('Error generating broker code:', xhr.responseText || xhr.statusText);
				alert_float('error', 'Error generating broker code. Please try again.');
				$('#AccountID').val('');
				$('#HiddenBrokerCode').val('');
			},
			complete: function() {
	...
	$('.saveBtn').on('click', function() {
		...
		// } 
			...
			$.ajax({
				url: "<?php echo admin_url(); ?>Broker/SaveAccountID",
				dataType: "JSON",
				method: "POST",
				data: formData,
				processData: false,
	...
	$('.updateBtn').on('click', function() {
		...
		// } 
			...
			formDataUpd.append('AccountName', AccountName);
			...
			formDataUpd.append('vendor', vendor);
			formDataUpd.append('customer', customer);
			...
			$.ajax({
				url: "<?php echo admin_url(); ?>Broker/UpdateAccountID",
				dataType: "JSON",
				method: "POST",
				data: formDataUpd,
				processData: false,
