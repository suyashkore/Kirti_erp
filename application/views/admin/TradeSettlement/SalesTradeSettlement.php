          ...
          <div class="panel-body">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
                ...
                <li class="breadcrumb-item active" aria-current="page"><b>Sales Trade Settlement</b></li>
              </ol>
            </nav>
            <hr class="hr_style">
            <br>
              ...
              <div class="row">
                <div class="col-md-2 mbot5">
                  <div class="form-group" app-field-wrapper="PartyID">
                    <label for="PartyID" class="control-label"><small class="req text-danger">* </small>Party / Customer Name</label>
                ...
                <div class="col-md-2 mbot5">
                  <div class="form-group" app-field-wrapper="OrderID">
                    ...
                    <select name="OrderID" id="OrderID" class="form-control selectpicker" data-live-search="true" app-field-label="Booking ID" onchange="getBookingListDetails(this.value)">
                      <option value="" selected>None selected</option>
                    </select>
                  </div>
                </div>
                ...
                <div class="col-md-2 mbot5">
                  <div class="form-group" app-field-wrapper="InwardWt">
                    <label for="InwardWt" class="control-label"><small class="req text-danger">* </small>Dispatched Weight(KG)</label>
                    <input type="text" name="InwardWt" id="InwardWt" class="form-control" app-field-label="Inward Weight" readonly>
                  </div>
                </div>
                <div class="col-md-2 mbot5">
                  <div class="form-group" app-field-wrapper="UnitRate">
                    <label for="UnitRate" class="control-label"><small class="req text-danger">* </small>Unit Rate</label>
                    <input type="text" name="UnitRate" id="UnitRate" class="form-control" app-field-label="Todays Rate" readonly>
                  </div>
                </div>
                <div class="col-md-2 mbot5">
                  <div class="form-group" app-field-wrapper="BalanceQty">
                    <label for="BalanceQty" class="control-label"><small class="req text-danger">* </small>Balance Quantity</label>
                    <input type="text" name="BalanceQty" id="BalanceQty" class="form-control" app-field-label="Shortage Quantity " readonly>
                  </div>
                </div>
                <div class="col-md-2 mbot5">
                  <div class="form-group" app-field-wrapper="BalanceAmt">
                    <label for="BalanceAmt" class="control-label"><small class="req text-danger">* </small>Balance Amt</label>
                    <input type="text" name="BalanceAmt" id="BalanceAmt" class="form-control" app-field-label="Shortage Amount">
                  </div>
                </div>
                <div class="col-md-2 mbot5">
                ...
                <div class="col-md-2 mbot5">
                  <div class="form-group" app-field-wrapper="ShortageInvoice">
                    <label for="ShortageInvoice" class="control-label">Generate Balance Invoice</label>
                  ...
                  <table width="100%" class="table" id="items_table">
                    <thead>
                      <tr style="text-align: center;">
                        ...
                        <th style="width:100px;">Delivery Order Date</th>
...
$(document).ready(function(){
  ...
  function getBookingDetails(callback = null) {
    ...
    if (!PartyID) {
      $('#OrderID').html('<option value="" selected>None selected</option>');
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        let data = response.customer_data || [];
        ...
        $.each(data, function(index, item) {
          // Example: show OrderID + DeliveryOrderID for better clarity
          html += `<option value="${item.OrderID}">${item.OrderID}</option>`;
        });
    ...
    $.ajax({
      ...
      success: function(response) {
        if (!response.success) return;
          ...
          let d = response.sales_data;
          let i = response.delivery_data;

          $('#BookingDate').val(moment(d.TransDate).format('DD/MM/YYYY'));
          $('#BookingRate').val(d.NetAmt);
          $('#UnitRate').val(d.ItemAmt);
          $('#BookingWt').val(d.TotalWeight);
          $('#InwardWt').val(i.TotalWt);

          $('#BalanceAmt').val(d.NetAmt - i.TotalOrderAmt);
          $('#BalanceQty').val(d.TotalQuantity - i.TotalOrderQty);
          ...
          let items = i.delivery_data; // array of delivery rows
          ...
          if (items && items.length > 0) {
            items.forEach(function(item) {
              ...
              $('#order_id' + row).text(item.TransID || '');
              $('#gate_pass' + row).text(item.GatePass || '');
              $('#inward_date' + row).text(item.TransDate ? moment(item.TransDate).format('DD/MM/YYYY') : '');
              $('#account_id' + row).text(item.AccountID || '');
              $('#party_name' + row).text(item.company || '');
              $('#booking_id' + row).text(item.OrderID || '');
              $('#item_id' + row).text(item.ItemID || '');
              $('#item_name' + row).text(item.item_name || '');
              $('#total_weight' + row).text(item.TotalWeight || 0);

              // Per-row weights — use item-level if available, else fallback to summary
              $('#total_bag' + row).text(item.BagWeight || i.BagWeight || 0);
              $('#total_katta' + row).text(item.TotalKatta || i.TotalKatta || 0);
              $('#total_layer' + row).text(item.TotalLayer || i.TotalLayer || 0);
              $('#status' + row).text(item.gate_Status || i.gate_Status || '');
            });
            calculateTotals();
  ...
  function calculateShortage() {
    ...
    let rate = parseFloat($('#UnitRate').val()) || 0;

    // Calculate shortage quantity
    let BalanceQty = bookingWt - inwardWt;

    if (BalanceQty < 0) BalanceQty = 0;

    // Calculate shortage amount
    let BalanceAmt = BalanceQty * rate;

    $('#BalanceQty').val(BalanceQty.toFixed(2));
    $('#BalanceAmt').val(BalanceAmt.toFixed(2));
  }
