...
class Reports extends AdminController
{
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($proposalsTaxesSelect as $key => $tax) {
                array_splice($select, 8, 0, '(
                    ...
                    WHERE ' . db_prefix() . 'itemable.rel_type="proposal" AND taxname="' . $tax['taxname'] . '" AND taxrate="' . $tax['taxrate'] . '" AND ' . db_prefix() . 'itemable.rel_id=' . db_prefix() . 'proposals.id AND ' . db_prefix() . 'itemable.is_selected=1) as total_tax_single_' . $key);
            }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($estimatesTaxesSelect as $key => $tax) {
                array_splice($select, 9, 0, '(
                    ...
                    FROM ' . db_prefix() . 'itemable
                    INNER JOIN ' . db_prefix() . 'item_tax ON ' . db_prefix() . 'item_tax.itemid=' . db_prefix() . 'itemable.id
                    WHERE ' . db_prefix() . 'itemable.rel_type="estimate" AND taxname="' . $tax['taxname'] . '" AND taxrate="' . $tax['taxrate'] . '" AND ' . db_prefix() . 'itemable.rel_id=' . db_prefix() . 'estimates.id AND ' . db_prefix() . 'itemable.is_selected=1) as total_tax_single_' . $key);
            }
    ...
    public function items()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $where = ['AND rel_type="invoice"', 'AND status != 5', 'AND status=2', 'AND is_selected=1'];
