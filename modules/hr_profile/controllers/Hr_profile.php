		...
		*/
		...
		public function GetAllStaffList()
		{
			...
			foreach($AccountDetails as $key=>$val){
				...
				$html .= "<td>".(($val["active"] == "1") ? "Active" : "DeActive")."</td>";
				$html .= "</tr>";
			}
			echo json_encode($html);
		}
		...
		*/
		...
		public function load_data_table(){
			...
			if($this->input->post('hr_profile_report_to') != ''){
				...
				$this->db->where('tblstaff.team_manage',$data['hr_profile_report_to']??null);  
				$query = $this->db->get();
			...
			if(count($data) > 0){
				...
				foreach($data as $aRow){
					...
					$html.= '<td class="sorting_1">'.$aRow['firstname'].' '.$aRow['lastname'].'</td>';
