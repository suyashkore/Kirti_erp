...
<script>
	$(function(){
		...
		var StaffServerParams = {
			...
			"hr_profile_state": "[name='hr_profile_state']",
			"hr_profile_report_to": "[name='hr_profile_report_to']",
			"staff_role": "[name='staff_role[]']",
			"staff_type": "[name='staff_type[]']",
			"staff_teammanage": "input[name='staff_dep_tree']",
			"status": "[name='status']",
		};
		var table_staff = $('table.table-table_staff');
		initDataTable(table_staff,admin_url + 'hr_profile/table', [0],[0], StaffServerParams, [0, 'ASC']);
		...
		$.each(StaffServerParams, function() {
			$('#hr_profile_state').on('change', function() {
				table_staff.DataTable().ajax.reload()
				.columns.adjust()
				.responsive.recalc();
			});
		...
		$.each(StaffServerParams, function() {
			$('#hr_profile_report_to').on('change', function() {
				table_staff.DataTable().ajax.reload()
				.columns.adjust()
				.responsive.recalc();
			});
