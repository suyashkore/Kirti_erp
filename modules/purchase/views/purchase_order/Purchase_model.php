...
class Purchase_model extends App_Model
{
	...
	*/
	...
	public function add_pur_order_po($data)
	{
		...
		if ($this->db->affected_rows() > 0) {
			...
			foreach ($es_detail as $value) {
				...
				$VenItem = $this->db->get_where(db_prefix() . 'VendorWiseItems', array('ItemID' => $item_c->ItemID, 'AccountID' => $AccountID))->row();

				$OldRate = $this->get_last_purch_order_detail_old_item($item_c->ItemID);
	...
	public function update_purchase_order_PO($data, $id)
	{
		...
		if ($this->db->affected_rows() > 0) {
			...
			foreach ($es_detail as $value) {
				...
				$VenItem = $this->db->get_where(db_prefix() . 'VendorWiseItems', array('ItemID' => $item_c->ItemID, 'AccountID' => $data['vendor']))->row();
	...
	public function get_vendor_name()
	{
		...
		$this->db->from('tblclients');
		$this->db->join(db_prefix() . 'AccountSubGroup2', 
