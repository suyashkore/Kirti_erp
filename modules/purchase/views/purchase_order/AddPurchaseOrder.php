...
<style>
	...
	.select-readonly .dropdown-toggle {
	...
	#page-loader-overlay .loader-center {
		position: absolute;
		top: 50%;
		left: 50%;
			...
			<div class="modal-body">
				<div class="row mb-2">
					...
					<div class="col-md-2 mbot5">
						<div class="form-group" app-field-wrapper="filter_purchase_location">
							<label for="filter_purchase_location" class="control-label"><small
									class="req text-danger"> </small> Purchase Location</label>
							<select name="filter_purchase_location" id="filter_purchase_location"
						...
						<form action="<?= admin_url('purchase/savepurchase'); ?>" method="post" id="main_save_form">
							...
							<div class="row">
											...
											<div class="row">
												...
												<div class="col-md-4">
													<div class="form-group">
														...
														<input type="text" name="vendor_doc_no" id="vendor_doc_no" class="form-control" placeholder="Enter vendor doc number">
													</div>
												</div>
												<div class="col-md-4">
												...
												<div class="col-md-4">
													<div class="form-group">
														...
														<select name="payment_terms" id="payment_terms" class="form-control selectpicker" data-live-search="true" required>
															<option value=""></option>
												...
												<div class="col-md-4">
													<div class="form-group" app-field-wrapper="freight_terms">
														...
														<select name="freight_terms" id="freight_terms" class="form-control selectpicker" data-live-search="true" app-field-label="Freight Terms" required>
															<option value="" selected>None selected</option>
															<?php
												...
												<div class="col-md-4">
													<div class="form-group">
														<label>Broker</label>
														<select name="broker" id="broker" class="form-control selectpicker" data-live-search="true" required>
															<option value="" selected>None selected</option>
															<?php
															if (!empty($Broker)):
												...
												<div class="col-md-4">
													<div class="form-group">
														...
														<span id="attachment_link_container" style="margin-left:10px;"></span>
												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<textarea name="internal_remarks" id="internal_remarks" class="form-control" rows="2" placeholder="Enter internal remarks..."></textarea>
													</div>
												</div>
												<div class="col-md-6">
													<div class="form-group">
														<label>Document Remark</label>
														<textarea name="document_remark" id="document_remark" class="form-control" rows="2" placeholder="Enter document remarks..."></textarea>
													</div>
												</div>
											</div>
										</div>
								...
								<div class="col-md-12" style="position: fixed; bottom: 0; left: 0; right: 0; background: #fff; padding: 10px 20px 10px 0px; margin-top: 10px; box-shadow: 0 -2px 0px rgba(0,0,0,0.1); z-index: 2; text-align: right;">
									<button type="submit" class="btn btn-success saveBtn <?= (has_permission_new('PurchaseOrder', '', 'create')) ? '' : 'disabled'; ?>">
										<i class="fa fa-save"></i> Save
									</button>
									<button type="button" class="btn btn-primary printPdfBtn updateBtn" style="display: none; margin-right:8px;" onclick="printPurchaseOrderPdf();">
										<i class="fa fa-print"></i> Print PDF
									</button>
									<button type="submit" class="btn btn-success updateBtn <?= (has_permission_new('PurchaseOrder', '', 'edit')) ? '' : 'disabled'; ?>" style="display: none;">
...
<script>
	$(document).ready(function() {
		...
		$('#quotation_date').datetimepicker({ format: 'd/m/Y', minDate: minStartDate, maxDate: maxEndDate, timepicker: false });
		...
		$('#vendor_quote_date').datetimepicker({ format: 'd/m/Y', minDate: minStartDate, maxDate: maxEndDate, timepicker: false });
		$('#from_date_modal').datetimepicker({ format: 'd/m/Y', minDate: minStartDate, maxDate: maxEndDate, timepicker: false });
		$('#to_date_modal').datetimepicker({ format: 'd/m/Y', minDate: minStartDate, maxDate: maxEndDate, timepicker: false });

		// =============================================
		// ✅ URL  edit_id   auto edit mode
		// =============================================
		var urlParams = new URLSearchParams(window.location.search);
		var editId = urlParams.get('edit_id');
		if (editId) {
			// Page load   getDetails  
			// (selectpicker व datetimepicker initialize   )
			setTimeout(function() {
				getDetails(editId);
			}, 500);
		}
	...
	function showLoader(msg) {
		...
		$('#page-loader-overlay').addClass('active');
	}
	function hideLoader() {
		$('#page-loader-overlay').removeClass('active');
	}

	// =============================================
	// ✅ HELPER: Vendor Quote No - READONLY SET
	// =============================================
	function setVendorQuoteNoReadonly(isReadonly) {
		if (isReadonly) {
			$('#vendor_quote_no').prop('disabled', true);
			$('#vendor_quote_no').closest('.bootstrap-select').addClass('select-readonly');
			$('#vendor_quote_no').closest('.bootstrap-select').find('.dropdown-toggle').prop('disabled', true);
		} else {
			$('#vendor_quote_no').prop('disabled', false);
			$('#vendor_quote_no').closest('.bootstrap-select').removeClass('select-readonly');
			$('#vendor_quote_no').closest('.bootstrap-select').find('.dropdown-toggle').prop('disabled', false);
		}
	...
	function ResetForm() {
		...
		$('#attachment').show();
		$('#vendor_quote_no').data('selected-val', '');
		setVendorQuoteNoReadonly(false);

		// ✅ URL  edit_id   (Reset  clean URL)
		if (window.history && window.history.replaceState) {
			var cleanUrl = "<?= admin_url('purchase/AddPurchaseOrder'); ?>";
	...
	function getNextQuotationNo(callback = null) {
		...
		if ($('#form_mode').val() == 'add') {
			$.ajax({
				...
				dataType: 'json',
				success: function(response) {
					if (response.success == true) { $('#order_no').val(response.order_no); }
					else { $('#order_no').val(''); }
				},
	...
	function getVendorDetailsLocation(callback = null) {
		...
		$.ajax({
			...
			dataType: 'json',
			success: function(response) {
				if (response.status === 'success') { $('#currency').val(response.data.currency_name); }
			}
		});
	}
	...
	// =============================================
	// ✅ FORM SUBMIT - vendor_quote_no  mode  pass
	// =============================================
	$('#main_save_form').on('submit', function(e) {
		...
		if (items.length === 0) { alert_float('warning', 'Please add at least one item.'); return; }
		...
		// ✅ disabled field - data attribute  manually set 
		var quoteNoVal = $('#vendor_quote_no').data('selected-val') || $('#vendor_quote_no').val() || '';
		form_data.set('vendor_quote_no', quoteNoVal);
		...
		if (form_mode == 'edit') {
			...
			$.ajax({
				...
				success: function(response) {
					if (response.success == true) { alert_float('success', response.message); ResetForm(); }
					else { alert_float('warning', response.message); }
				}
			});
		} else {
			$.ajax({
				...
				complete: function() { $('button[type=submit]').attr('disabled', false); },
				success: function(response) {
					if (response.success == true) { alert_float('success', response.message); ResetForm(); loadPurchaseOrderList(); }
					else { alert_float('warning', response.message); }
				}
			});
		}
	...
	function getDetails(id) {
		ResetForm();
		showLoader('Fetching purchase order details...');
		$.ajax({
			...
			success: function(response) {
				if (response.success == true) {
					...
					getCustomDropdownList('item_type', d.ItemType, 'item_category', d.ItemCategory, function() {
						...
						getNextQuotationNo(function() {
							if (d.QuatationID) {
								...
								$('#vendor_quote_no').data('selected-val', d.QuatationID);
								$('#vendor_quote_no').selectpicker('refresh');
							}
							$('#items_body').empty();
							$('#row_id').val(0);
					...
					if (d.Attachment) {
						...
						$('#attachment_link_container').html('<a href="' + attachUrl + '" target="_blank" style="font-weight:bold; color:#007bff; text-decoration:underline;">View</a>');
					} else {
						$('#attachment_link_container').html('');
					}
					...
					setTimeout(function() { setVendorQuoteNoReadonly(true); }, 400);
			...
			error: function() {
				hideLoader();
				alert_float('danger', 'Error fetching purchase order details.');
			}
		});
	}

	// =============================================
	// CLICK HANDLER FOR LIST MODAL ROWS
	// =============================================
	$(document).on('click', '#table_ListModal .get_Details', function() {
		var id = $(this).data('id');
		if (id) { getDetails(id); $('#ListModal').modal('hide'); }
	});
</script>
...
<script>
	...
	function showPurchaseOrderList() {
		...
		setTimeout(function() { $('#filter_category').selectpicker('refresh'); }, 300);
	...
	function loadPurchaseOrderList() {
		...
		var location = $('#filter_purchase_location').val();
		$('#table_ListModal tbody').html('<tr><td colspan="13" class="text-center"><i class="fa fa-spinner fa-spin"></i> Loading...</td></tr>');

		var statusMap = {
			1: { label: 'Pending' }, 2: { label: 'Cancel' }, 3: { label: 'Expired' },
			4: { label: 'Approved' }, 5: { label: 'In-Progress' }, 6: { label: 'Completed' },
			7: { label: 'Partially Completed' }
		};

		$.ajax({
			url: '<?= admin_url('purchase/get_purchase_order_data'); ?>',
			type: 'GET',
			data: { from_date: from_date, to_date: to_date, category: category, location: location },
			dataType: 'json',
			success: function(data) {
				var html = '';
				if (Array.isArray(data) && data.length > 0) {
					$.each(data, function(i, row) {
						var statusKey = parseInt(row.Status);
						var statusInfo = statusMap[statusKey] || { label: 'Unknown' };
						html += `<tr class="get_Details order-main-row" data-id="${row.id}" style="cursor:pointer;">
							<td>${row.PurchID || ''}</td>
							<td>${row.CategoryName || ''}</td>
							<td>${row.LocationName || ''}</td>
							<td  class="fixed-td" title="${row.company || ''}" >${row.company || ''}</td>
					...
					html = '<tr><td colspan="13" class="text-center text-muted">No records found for selected filter.</td></tr>';
				}
				$('#table_ListModal tbody').html(html);
			},
			error: function() {
				$('#table_ListModal tbody').html('<tr><td colspan="13" class="text-center text-danger">Error loading data. Please try again.</td></tr>');
			}
		});
	}
	...
	$(document).on('click', '#show_filter_btn', function() {
		var from_date = $('#from_date_modal').val();
		var to_date = $('#to_date_modal').val();
		if (!from_date || !to_date) { alert_float('warning', 'Please select both From Date and To Date.'); return; }
		var from_m = moment(from_date, 'DD/MM/YYYY');
		var to_m = moment(to_date, 'DD/MM/YYYY');
		if (from_m.isAfter(to_m)) { alert_float('warning', 'From Date cannot be greater than To Date.'); return; }
		loadPurchaseOrderList();
	});
	...
	function onVendorQuoteNoChange(quoteId) {
		$('#vendor_quote_no').data('selected-val', quoteId);
		if (!quoteId) { $('#vendor_quote_date').val(''); return; }
		$.ajax({
			url: '<?= admin_url('purchase/Quotation/GetQuotationDetailsitem'); ?>',
			type: 'POST',
			data: { id: quoteId },
			dataType: 'json',
			success: function(response) {
				if (response.success === true) {
					...
					$('#purchase_location').val(d.PurchaseLocation);
					$('#vendor_location').val(d.DeliveryLocation);
					if (d.BrokerID) { $('#broker').val(d.BrokerID).selectpicker('refresh'); }
					$('#delivery_from').val(moment(d.DeliveryFrom).format('DD/MM/YYYY'));
					$('#delivery_to').val(moment(d.DeliveryTo).format('DD/MM/YYYY'));
					$('#payment_terms').val(d.PaymentTerms);
					$('#freight_terms').val(d.FreightTerms);
					$('.selectpicker').selectpicker('refresh');
				} else { $('#vendor_quote_date').val(''); }
			},
			error: function() { $('#vendor_quote_date').val(''); }
		});
	}
	...
	$(document).ready(function() {
		if ($.fn.datetimepicker) {
			$('#from_date_modal, #to_date_modal').datetimepicker({
				format: 'd/m/Y', timepicker: false, scrollMonth: false, scrollInput: false
			});
		}
	});
</script>
...
<style>
	.get_Details:hover { background-color: #e8f4ff !important; cursor: pointer; }
</style>