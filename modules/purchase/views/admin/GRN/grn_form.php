                                    ...
                                    <div>
                                        ...
                                        <div id="tab_other">
                                            ...
                                            <div class="row">
                                                ...
                                                <!-- ✅ ATTACHMENT SECTION WITH VIEW BUTTON -->
                                                <div class="col-md-4">
                                                    <div class="form-group">
                                                        <label class="control-label">
                                                            Attachment
                                                            <!-- ✅ View button - only shown when attachment exists -->
...
<script>
    ...
    function resetGRNForm() {
        ...
        $('#attachment_view_btn').hide().attr('href', '#'); // ✅ View button reset
   ...
   // ✅ GRN No re-fetch after reset (no page reload)
        $.ajax({
...
if (tdsVal === 'Yes' || tdsVal === 'Ye') {
    ...
    $('#tds_percent').val('');
            ...
            // ── Select dropdowns ──────────────────────────────────
            var simpleSelects = ['location', 'grn_status', 'business_unit',
                                 'freight_payable_to', 'transporter_id'];
            ...
            // ── Vendor ID → load locations & PO list ──────────────
            if (grn.vendor_id) {
    ...
    // ========================
// LOCATION CHANGE → PARTIAL RESET
// ========================
$('#location').on('change', function () {
    var selectedLocation = $(this).val();

    // Vendor reset
    $('#vendor_id').val('').selectpicker('refresh');
    $('#vendor_gst_no').val('');
    $('#vendor_country').val('');
    $('#vendor_state').val('');
    $('#vendor_address').val('');

    // Vendor location reset
    $('#vendor_location').html('<option value="" selected disabled>None selected</option>');
    $('#vendor_location').selectpicker('refresh');
