                                        ...
                                        <div class="table-GRN_List tableFixHead2">
                                            <table class="tree table table-striped table-bordered" id="table_GRN_List" width="100%">
                                                ...
                                                <tbody id="grnlistbody">
                                                    <tr><td colspan="6" class="text-center">Click "Show All" to load GRN list</td></tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
...
$('#transporter_id').on('change', function () {
    var tid = $(this).val();
    if (!tid) { $('#transporter_pan').val(''); return; }
    $.ajax({
...
$('.cancelBtn').on('click', function () { resetGRNForm(); });
...
$('.showAllBtn').on('click', function () {
    ...
    $('#GRN_List').on('shown.bs.modal', function () { $('#grnSearchInput').val('').focus(); });
});
...
function populateGRNData(data) {
    try {
        ...
        if (grn.tds_freight === 'Yes') { $('#tds_code_wrap').show(); }
...
.req { font-size: 12px; }
.p_style { color: #ee5770; margin-bottom: 5px; margin-top: 5px; }
.hr_style { border-top: 2px solid #50607b; margin-top: 5px; margin-bottom: 10px; }
</style>