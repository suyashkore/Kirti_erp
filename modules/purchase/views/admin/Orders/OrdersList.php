          ...
          <div class="panel-body">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb custombreadcrumb" style="background-color:#fff !important; margin-Bottom:0px !important;">
                ...
                <li class="breadcrumb-item active" aria-current="page"><b>Order List</b></li>
              </ol>
            </nav>
            <hr class="hr_style">
              ...
              <div class="row">
                ...
                <div class="col-md-2 mbot5">
                  <div class="form-group" app-field-wrapper="filter_purchase_location">
                    <label for="filter_purchase_location" class="control-label"><small class="req text-danger"> </small> Purchase Location</label>
                    <select name="filter_purchase_location" id="filter_purchase_location" class="form-control selectpicker" data-live-search="true" app-field-label="Purchase Location">
                      <option value="" selected>All</option>
                      <?php
                ...
                <div class="col-md-9 mbot5" style="padding-top: 20px;">
                  ...
                  <?php
                  if (has_permission_new('PurchaseOrderList', '', 'export')) {
                  ...
                  if (has_permission_new('PurchaseOrderList', '', 'print')) {
...
<script>
  ...
  $('#filter_list_form').submit(function(e) {
    ...
    function fetchChunk() {
      ...
      $.ajax({
        url: "<?= admin_url('purchase/Orders/ListFilter') ?>",
        type: "POST",
        data: form_data,
        processData: false,
        contentType: false,
  ...
  function appendRows(rows) {
    ...
    rows.forEach(function(row) {
      html += `<tr class="clickable-row" data-id="${row.id}" style="cursor:pointer;">
        <td class="text-center">${row.PurchID}</td>
        <td>${moment(row.TransDate).format('DD/MM/YYYY')}</td>
        <td>${row.category_name}</td>
        <td>${row.LocationName}</td>
        <td>${row.QuatationID}</td>
...
$(document).on('click', '#table-list tbody tr.clickable-row', function() {
  var id = $(this).data('id');
  if (id) {
    var url = "<?= admin_url('purchase/AddPurchaseOrder'); ?>?edit_id=" + id;
  ...
  function exportTableToExcel() {
    ...
    $.ajax({
      url: "<?= admin_url('purchase/Orders/ListExportExcel') ?>",
      type: 'POST',
      data: formData,
      dataType: 'json',
      processData: false,
