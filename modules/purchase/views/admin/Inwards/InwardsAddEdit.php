...
/* ===== FULL PAGE LOADER OVERLAY (Quotation style) ===== */
#autoLoadOverlay {
  display: none;
  position: fixed;
  top: 0;
...
// ===== filter list form auto-submit on page load =====
$(document).ready(function() {
  setTimeout(() => {
...
// ===== AUTO LOAD FROM URL PARAMETER (from Inwards Report page click) =====
$(document).ready(function () {
  var urlParams = new URLSearchParams(window.location.search);
  var loadId = urlParams.get('load_id');
  if (loadId && loadId !== '') {
    // Wait for filter list to load first, then auto-load details
    // We use a flag so it runs only once after list finishes
    window._autoLoadId = parseInt(loadId);
  }
});
...
function getDetails(id, row){
  ...
  showLoader('Fetching inward details...');
    ...
    $.ajax({
      ...
      success: function(res){
        ...
        if(loadedRecords >= totalRecords){
          ...
          // ===== AUTO LOAD: URL parameter असल्यास load करा =====
          if(window._autoLoadId){
            var autoId = window._autoLoadId;
            window._autoLoadId = null;
            showLoader('Loading Inward Record...');
            getDetails(autoId, $('<tr>'));
          }
