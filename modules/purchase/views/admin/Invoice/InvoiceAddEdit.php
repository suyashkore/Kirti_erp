				...
				<div class="row mb-2">
					...
					<div class="col-md-2">
						<div class="form-group" app-field-wrapper="to_date_modal">
							<?= render_date_input('to_date_modal', 'To Date', date('01/m/Y'), []); ?>
						</div>
						...
						<form action="<?= admin_url('purchase/Invoice/UpdateInvoice'); ?>" method="post" id="main_save_form" enctype="multipart/form-data">
							...
							<div class="row">
								<div class="col-md-12" style="position: fixed; bottom: 0; left: 0; right: 0; background: #fff; padding: 10px 20px 10px 0px; margin-top: 10px; box-shadow: 0 -2px 0px rgba(0,0,0,0.1); z-index: 2; text-align: right;">
									...
									<button type="button" class="btn btn-success saveBtn <?= (has_permission_new('PurchaseInvoice', '', 'create')) ? '' : 'disabled'; ?>" id="SaveBtn"><i class="fa fa-save"></i> Save</button>
									<button type="submit" class="btn btn-success updateBtn <?= (has_permission_new('PurchaseInvoice', '', 'edit')) ? '' : 'disabled'; ?>" style="display: none;"><i class="fa fa-save"></i> Update</button>
...
<script>
  $(document).ready(function() {
	...
	$("#SaveBtn").on('click', function() {
		...
		formData.append("vendor_doc_no", $("#vendor_doc_no").val());
		formData.append("VendorDocDate", $("#VendorDocDate").val());
		formData.append("vendor_doc_amount", $("#vendor_doc_amount").val());
		formData.append("vendor_dispatch_weight", $("#vendor_dispatch_weight").val());
		formData.append("internal_remarks", $("#internal_remarks").val());
		formData.append("document_remark", $("#document_remark").val());
		formData.append("vendor_state", $("#vendor_state").val());
		formData.append("Company_state", $("#Company_state").val());
		formData.append("hold_payment", $("#hold_payment").val());
