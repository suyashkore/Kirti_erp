...
class Mandi_model extends App_Model
{
    ...
    public function get_po_no()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $number  = $number + 1;
            return 'MPO' . $FY . $PlantID . str_pad($number, 5, '0', STR_PAD_LEFT);
        } else {
    ...
    public function get_vendor_terms($vendor_id)
    {
        ...
        if ($query->num_rows() > 0) {
            return ['success' => true, 'data' => $query->row_array()];
        } else {
            return [
                'success' => false,
    ...
    public function save_mandi_purchase($purchase_data)
    {
        try {
            ...
            // Parse date format (already Y-m-d from controller)
            $inwards_date = $purchase_data['inwards_date'];
            ...
            } elseif ($purchase_data['form_mode'] == 'edit') {
    ...
    {
        $required_fields = [
            'purchase_order', 'inwards_date', 'location_id',
            'godown_id', 'item_id_header', 'form_json'
        ];
    ...
    private function _insert_mandi_purchase($purchase_data, $inwards_date, $items)
    {
        ...
        // Insert line items + ledger entries
        $this->_insert_mandi_purchase_items(
            $mandi_purchase_id,
            $items,
            $purchase_data['purchase_order'],
            $purchase_data['user'],
            $purchase_data['item_id_header'],
            $inwards_date   // ✅ 6th argument
        );
    ...
    private function _update_mandi_purchase($purchase_data, $inwards_date, $items)
    {
        ...
        // ✅ STEP 1 - Old Purchase Details delete 
        $this->db->where('PurchID', $purchase_data['purchase_order'])
                 ->delete('tblMandiPurchaseDetails');

        // ✅ STEP 2 - Old Ledger Entries delete 
        $this->db->where('VoucherID', $purchase_data['purchase_order'])
                 ->where('PassedFrom', 'PURCHASE')
                 ->delete('tblaccountledger');

        // ✅ STEP 3 -  line items + ledger entries insert   call)
        $this->_insert_mandi_purchase_items(
            $update_id,
            $items,
            $purchase_data['purchase_order'],
    ...
    /**
     * Insert Mandi Purchase line items + Ledger entries
     *
     ...
     * @param string $inwards_date      - YYYY-MM-DD format
     */
    private function _insert_mandi_purchase_items(
        $mandi_purchase_id,
        $items,
        $purchase_order,
        $user_id,
        $item_id,
        $inwards_date       // ✅ 6th argument - required
    ) {
        $PlantID = $this->session->userdata('root_company');
        $FY      = $this->session->userdata('finacial_year');
        ...
        foreach ($items as $item) {
            ...
            // ✅ Ledger entries insert करा
            $this->_insert_ledger_entries($item, $purchase_order, $user_id, $inwards_date);
        }
    }
    ...
    /**
     * Insert Account Ledger entries for each Mandi Purchase line item
     *  item  4 rows insert  tblaccountledger 
     *
     * @param array  $item
     * @param string $purchase_order
     * @param int    $user_id
     * @param string $inwards_date    - YYYY-MM-DD format
     */
    private function _insert_ledger_entries($item, $purchase_order, $user_id, $inwards_date)
    {
        $PlantID = $this->session->userdata('root_company');
        $FY      = $this->session->userdata('finacial_year');

        $vendor_id    = isset($item['vendor_id'])   ? $item['vendor_id']   : '';
        $net_amt      = isset($item['net_amt'])      ? $item['net_amt']     : 0;
        $brokerage    = isset($item['brokerage'])    ? $item['brokerage']   : 0;
        $market_levy  = isset($item['market_levy'])  ? $item['market_levy'] : 0;
        $gross        = isset($item['gross'])        ? $item['gross']       : 0;
        $qty        = isset($item['qty_quintal'])        ? $item['qty_quintal']       : 0;
        $btotal= $brokerage * $qty;
        $ltotal= $market_levy * $qty;

        $current_datetime = date('Y-m-d H:i:s');
        $narration        = 'Mandi Purchase ' . $purchase_order . ' for Vendor ' . $vendor_id;

        $ledger_rows = [

            // ROW 1 - Vendor Credit (Net Amount)
            [
                'PlantID'    => $PlantID,
                'FY'         => $FY,
                'Transdate'  => $inwards_date,
                'VoucherID'  => $purchase_order,
                'TransDate2' => $current_datetime,
                'UniquID'    => '',
                'AccountID'  => $vendor_id,
                'EffectOn'   => 'PURCH',
                'TType'      => 'C',
        ...
        foreach ($ledger_rows as $row) {
            if (!$this->db->insert('tblaccountledger', $row)) {
                throw new Exception(
                    'Failed to insert ledger entry for Vendor: ' . $vendor_id .
    ...
    public function getMandiList($from_date = null, $to_date = null, $filter_location = null, $filter_godown = null, $filter_item = null)
    {
        ...
        if (!empty($from_date))       { $this->db->where('DATE(m.TransDate) >=', $from_date); }
        if (!empty($to_date))         { $this->db->where('DATE(m.TransDate) <=', $to_date); }
        if (!empty($filter_location)) { $this->db->where('m.CenterLocation', $filter_location); }
        if (!empty($filter_godown))   { $this->db->where('m.WarehouseID', $filter_godown); }
        if (!empty($filter_item))     { $this->db->where('m.ItemID', $filter_item); }

        $this->db->order_by('m.id', 'ASC');
        return $this->db->get()->result_array();
    }

    public function getMandiById($id)
    {
        $this->db->select('
            m.id, m.OrderID, m.OrderDate, m.CenterLocation, m.WarehouseID,
            m.ItemType, m.ItemID, m.TDSCode, m.VehicleNo,
            m.TotalQty, m.TotalValue, m.TotalBrokerage, m.TotalMarketPrice,
            m.TotalGrossValue, m.TDSAmt, m.FinalAmt, m.TransDate
        ');
        $this->db->from('tblMandiPurchaseMaster m');
    ...
    public function GetMandiDetailsallPDF($id, $order_id)
    {
        $this->db->select('
            ...
            m.LeaderGroupID, m.LeaderID, m.TotalQty, m.TotalValue,
            m.TotalBrokerage, m.TotalMarketPrice, m.TotalGrossValue,
            m.TDSAmt AS MasterTDSAmt, m.FinalAmt, m.TransDate,
            l.LocationName AS CenterLocation,
            g.GodownName   AS WarehouseID,
            i.ItemName     AS ItemID,
    ...
    {
        $this->db->select('
            h.id as id, h.PurchID as MandiPurchaseID, h.DocNo as DocumentNo,
            h.Payment_term as PaymentTerm, h.bag as BagQty, h.wt_per_bag as WeightPerBag, h.loose_in_kg as LooseKG,
            h.OrderQty as QtyQuintal, h.PurchRate as RatePerQuintal, h.OrderAmt as Value, h.BrokerAmt as Brokerage,
            h.MrtLevyAmt as MarketLevy, h.Round_off as RoundOff, h.GrossAmt as Gross,
            h.TDSAmt AS HistoryTDSAmt, h.NetOrderAmt as NetAmt, h.LupDate as CreatedDate, h.UserID as CreatedBy,
